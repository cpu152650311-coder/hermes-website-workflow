...
name: 建站工作流
version: "11.1"
description: B2B外贸出口建站（通用）。支持新建模式（一句话→30+页完整站）和补充模式（审计已有站点→缺口报告→补页面/补内容/补图片）。ui-ux-pro-max专业配色、AI去背Logo、部署后审计。
upstream: https://github.com/cpu152650311-coder/website-workflow
---

# 建站工作流 v11.1

> **v11.1**：**视觉设计专业化 + Logo自动化 + 去模板化**。Phase 1B 整合 ui-ux-pro-max 获取专业配色/字体建议（161 调色板 + 57 字体配对），新增 Pinterest 视觉参考搜索获取品类真实 mood board。Logo 生成流程全自动：GPT Image 2 生图 → rembg AI 去背景 → Pillow 缩放 favicon 全尺寸。gen-site-images.py 新增 `--limit N` 参数支持样图阶段精确控制。Phase 0 研究增强：WebSearch + WebFetch 多维度搜索 + 竞品站信息架构分析。**去模板化**：删除 Hero CSS 代码块（改为设计原则），删除页面骨架表（改为信息维度要求），Agent 从零自主设计每个站的 CSS 和板块结构。
...
| 模式 | 触发 | 流程 |
|------|------|------|
| **新建** | "做个{品类}外贸站" | Phase 0→1A→1B→1C→2→3→4→5→6→7 |
| **补充** | "优化{站名}" / "{站名}内容不够" | Phase S0→S1→S2→S3→S4→S5 |

### 🎯 目标标准：huaxingpcba 级别

**每站的目标是产出 huaxingpcba.com 同等充实度的站点**——内容深度、页面广度、图片密度都达到同一量级。以下为具体指标：

| 指标 | 最低要求（硬性） | 目标值（应达到） | huaxingpcba 参考 |
|------|:--:|:--:|:--:|
| 非博客页面数 | ≥22 | 30-60 | ~60 页 |
| 博客文章 | ≥3 | 3 初始 + 后续扩展 | 36+ 篇 |
| 总图片数 | ≥50 | 80-150 | ~350 张(非博客) |
| 平均每页图片 | ≥1 | 2-4 | ~6 张/页 |
| 首页板块数 | ≥5 | 8-12 | 10 板块 |
| 首页图片数 | ≥3 | 8-12 | 12 张 |
| 每页正文词数 | 按架构表最低 | 最低×1.5-2 | 丰富 |
| SEO 覆盖率 | 每页 100% | 100% + JSON-LD 全类型 | 完整 |
| 安全基础设施 | _headers + robots + sitemap | + llms.txt + CSP | 完整 |

**Agent 行为准则**：
- 内容架构表中的「最低」是**保底线**，不是目标。Agent 应自然写出充实内容，不应「刚好够最低」就停
- 图片策略应倾向于**多而不是少**——板块有配图比纯文字有说服力。$0.006/张，不要省
- 能力页和行业页应**深度展开**——三级分类比两级好，6 个行业比 4 个好。不设上限
- 所有要求 huaxingpcba 做到的（内链交叉引用、数据可视化、案例展示等），新站也该做到
- **如果 Agent 不确定某个板块是否该加 → 加，而不是跳过**
...
Agent 调研完后，将品类分类整理为**树形结构**（不是平铺列表），呈现给用户确认。树的每个分支对应网站导航的一级/二级/三级菜单：

```
📋 网站结构方案 — 请确认或调整：

一、核心导航（能力分类）— 将成为主导航的下拉菜单

   Agent 根据品类调研，识别出以下自然分类树：
   ...
   每个一级分支将成为主导航的一个菜单项（带下拉或直接链接）。

二、行业方案导航 — 将成为主导航的独立菜单
...
五、核心信任页（以下为必需，基础 14 页 + 博客）
   ☑ about ☑ factory ☑ quality ☑ shipping 
   ☑ faq ☑ how-it-works ☑ design-guides ☑ projects
   ☑ contact ☑ privacy ☑ terms ☑ 404 ☑ blog
```

**树形结构原则**：
- Agent 必须识别品类的**自然分类层级**（一级/二级/三级），不是随意拆分
- 有二级子类的分支 → 一级页面成为 hub + 下拉菜单，每个二级有独立详情页
- 无子类的分支 → 单页，不强行拆分
- 三级分类只在该品类确实有深层技术细分时才展开（如 PCB→HDI→任意层互连）
- **行业方案默认 ≥6 个**（品类天然服务多个下游行业），多多益善

**导航映射**：
- 一级分支 = 主导航项（有子类则下拉，无子类则直接链接）
- 行业方案 = 独立主导航项
- 信任页 = 放在导航的 "About" 或 "Resources" 下拉中，或放在 footer

用户勾选/调整后，Agent 产出 `page-tree.json`——每页的标题、H1、路径、所属导航位置、父页面都已确定。
...
#### 第 2 层：能力页（按品类自然分类树展开，2-3 层深）

Agent 根据 Phase 0 步骤 2 的**树形分类**展开能力页：

- 每个一级分支 → `capabilities/{slug}/index.html`（hub 页，链接所有二级子页）
- 每个二级分支 → `capabilities/{slug}/{subslug}/index.html`（独立详情页）
- 有三级分类的 → `capabilities/{slug}/{subslug}/{subsubslug}/index.html`
- 无子类的分支 → 单一详情页，不强行拆分

**导航映射**：一级分支成为主导航菜单项，其二级子类为下拉菜单项。三级子类不出现在全局导航中，通过二级详情页的内链访问。

**页面数量**：由品类自然分类深度决定，不硬性规定。一个复杂度高的品类（如 PCB 制造）天然有 20+ 能力子页；一个简单品类（如标准紧固件）可能只有 5-8 个。**不为了凑页面数而制造不存在的分类。**

#### 第 3 层：行业页（默认 ≥6 个行业）

`industries/` 为 hub 页。每个行业一个独立页：

每行业独立页包含：行业痛点 → 技术要求 → 我们的方案 → 相关认证 → 案例 → CTA。

**为什么默认 6 个**：一个制造品类天然服务多个下游行业。如果品类确实只服务 3-4 个行业，不强行凑数——但需要确认调研是否遗漏。
...
- 能力页按品类自然分类树展开（不是随意堆页面），深度由品类复杂度决定
- 行业页从下游行业调研自动映射，默认 ≥6 个（品类天然服务多个行业）
- 全部页面规划写入 `page-tree.json`，每页预分配：标题/H1/meta description/关键词/内容角度/所需图片角色/导航位置
- **树形结构决定导航菜单**：一级分支 → 主导航项；二级分支 → 下拉菜单项；三级分支 → 页面内链
...
**总量：每站 50-80 张最低，80-150 张达标（30-60 页站点），成本 ~$0.30-0.90。**
...
| 展示方式 | 适用角色 | CSS 要点 |
|---------|---------|---------|
| **全幅 Hero** | hero-bg, sub-hero-bg | `object-fit:cover; object-position:center 30%`；**默认居中文字**（`align-items:center;text-align:center`），通过 `::before` 叠加半透明遮罩保证文字可读性（暗色主题用 `rgba(0,0,0,x)`，亮色主题用 `rgba(255,255,255,x)` 或省略） |
...
| 页面 | 必须覆盖的信息维度 | 最低内容量 |
|------|-------------------|:--:|
| **首页** | ① 我是谁（品类+定位，1秒识别） ② 我提供什么能力/产品（至少 2 个维度展示） ③ 凭什么信我（认证/产能/品质至少 2 个信任信号） ④ 我服务过哪些场景（应用案例） ⑤ 怎么联系（CTA） | ≥5 个视觉板块，≥500 词 |
| **关于页** | 公司规模（成立年份/人数/面积）+ 主营业务范围 + 核心竞争优势 + 团队/设备亮点 | ≥3 个板块，≥300 词 |
| **工厂页** | 产线数量/类型 + 设备清单（关键设备名称+能力参数）+ 车间环境 + 产能数据（月产能/年产能） | ≥3 个板块，≥300 词 |
| **品质页** | QC 流程（IQC→IPQC→OQC）+ 检测设备（具体型号/检测精度）+ 认证列表（含编号）+ 品质数据（良率/退货率）+ 材料溯源 | ≥4 个板块，≥400 词 |
| **能力 Hub** | 所有能力子类入口（图标+标题+简述）+ 至少 1 个信任信号 + CTA | 每子类 ≥50 字简述 |
| **能力详情页** | 技术参数表（具体数值）+ 工艺能力清单 + 材料选项 + 品质标准 + 交期/MOQ + 适用领域 + 相关能力链接 | ≥5 个信息区，≥400 词 |
| **行业 Hub** | 所有行业入口（图标+行业名+简述）+ 至少 1 个跨行业信任信号 + CTA | 每行业 ≥50 字简述 |
| **行业详情页** | 行业痛点（3-4 条）+ 技术/认证要求 + 我们的方案（具体能力匹配）+ 相关案例 + CTA | ≥4 个板块，≥350 词 |
| **物流出货页** | 包装标准（材料/方式）+ 运输方式（海运/空运/快递）+ 时效表（不同地区）+ 出货流程 + 全球配送覆盖图/说明 | ≥4 个信息区，≥300 词 |
| **FAQ 页** | 至少 12 个问答，覆盖：产品/交期/MOQ/付款/物流/售后/定制/样品/认证/技术/包装/退换货 | ≥12 个 FAQ，可折叠展开 |
| **合作流程页** | 至少 6 步流程（询盘→确认→打样→生产→质检→出货），每步有时间估算 + 客户需配合的事项 | ≥6 步，每步 ≥50 字 |
| **设计指南页** | 文件格式要求 + 设计规则（DFM）+ 参数推荐表 + 常见设计错误 + 模板下载 CTA | ≥4 个板块，≥350 词 |
| **案例展示页** | 至少 3 个项目，每个含：客户行业/需求/解决方案/成果/客户评语（如有） | ≥3 个完整案例 |
| **快速报价页** | 表单（品类/数量/规格/交期要求）+ 或价格计算器 + 典型价格参考范围 + 交期说明 | 表单 ≤6 字段 |
| **联系页** | 询盘表单（name/email/company/message）+ 地址/邮箱/电话 + 办公/工厂图 + FAQ 摘要 | 表单 3-4 字段 |
| **隐私政策** | 数据收集说明 + 使用目的 + Cookie 政策 + 第三方分享 + 用户权利 + 联系方式 | 标准模板（≥6 段） |
| **服务条款** | 服务范围 + 订单条款 + 付款条件 + 知识产权 + 责任限制 + 争议解决 | 标准模板（≥6 段） |

**内容量保障**：上表中每页的最低内容量是硬性要求。Phase 5 子 Agent 验收时逐页检查：板块数量、Word count、信息维度覆盖率。
...
- 所有页面的信息维度必须覆盖，但排版方式由 Agent 根据品类和品牌人格自主决定
...
- 段落结构由设计时确定（典型 8-12 段），每屏一个说服点：Trust→能力展示→品质→流程→案例→表单→FAQ→Sticky CTA（具体段落和顺序按品类和品牌人格调整）
...
=== 内容充分性检查（对照 Phase 1B B2B 内容架构表） ===
29. 逐页检查：必须覆盖的信息维度是否全部到位（每页有独立检查清单）
30. 逐页检查：板块数量是否 ≥ 最低要求
31. 逐页检查：Word count 是否 ≥ 最低要求
32. 首页板块数量 ≥ 5 个，正文 ≥ 500 词
33. 关于/工厂/品质/能力详情/行业详情/物流/设计指南各 ≥ 3-4 板块，≥ 300-400 词
34. FAQ ≥ 12 条问答、合作流程 ≥ 6 步、案例 ≥ 3 个完整项目
35. 全站总 HTML 页面数 ≥ 22（14 核心信任页 + 能力子页 + 行业子页）
...
- `.modal-image-overlay`：`position:absolute; inset:0; background:var(--modal-overlay-gradient)` — 渐变遮罩保证文字可读（深色主题用暗渐变，亮色主题用白渐变，颜色变量在 design-tokens.css 中定义）
...
Agent 根据 Phase 1B 确定的品牌人格和 design-tokens.css 中的颜色变量，**自主设计 Hero 的 CSS**。不提供代码模板。

**布局方向**（由品牌人格三轴决定）：
- **居中**：文字垂直水平居中，适合产品微距/抽象背景（高端精密型/技术先锋型）
- **靠下**：文字靠底部，上半部透出背景内容，适合车间广角图（规模产能型/工厂直营型）
- **左对齐**：文字左对齐、图右置，适合产品展示图

**技术约束**：
- 背景图 `object-fit:cover; object-position:center 30%`
- 所有颜色从 `design-tokens.css` 的 CSS 变量引用（`var(--xxx)`），不写硬编码色值
- 暗色主题：`::before` 用暗遮罩保证浅色文字可读
- 亮色主题：`::before` 用白遮罩或省略
- 遮罩透明度根据实际背景图明暗决定——先看图再定值
...
已有站点内容不达标时，用补充模式**不重建、只补缺**。Agent 先审计后修补，保留现有设计系统。

### 触发方式

用户说"优化{站名}"、"给{站名}补充内容"、"{站名}内容太少了"时，Agent 自动进入补充模式。

### 补充模式流程

```
Phase S0  审计已有站点
  ├─ 逐页对照 B2B 内容架构表 → 生成 gap-report.md
  │   ├─ 板块数量 ≥ 最低要求？
  │   ├─ Word count ≥ 最低要求？
  │   ├─ 信息维度全部覆盖？
  │   └─ 缺失页面（对照 page-tree 4 层标准：14 核心+能力子页+行业子页）
  ├─ 检查 SEO 基础设施：_headers / robots.txt / sitemap / JSON-LD / llms.txt
  └─ 检查图片：无图片的板块、图片角色缺失

Phase S1  呈现缺口报告 → 用户选择（🛑确认）
  ├─ 🔴 缺页面：{具体页面清单}
  ├─ 🟡 缺内容：{页面名}: 只有 X 板块（需 ≥Y）/ Z 词（需 ≥W）/ 缺 {维度}
  ├─ 🔵 缺图片：{页面名}: {板块} 无配图
  └─ 用户勾选要修复的项目（可全选或部分）

Phase S2  补生图（如需要）
  └─ python gen-site-images.py --prompts supplement.json --out ./generated
     --manifest image-manifest.json --limit {N}

Phase S3  逐页补充/重写
  ├─ 保留 design-tokens.css（不修改颜色/字体/间距系统）
  ├─ 保留已有 CSS 类名约定 —— 新增板块用一致的命名风格
  ├─ 新增页面：在现有 header/footer 导航中添加链接
  ├─ 扩展现有页面：在现有内容基础上增加板块，不删除已有内容
  └─ 新图片引用追加到 image-manifest.json

Phase S4  针对修改页面的 10 秒买家测试

Phase S5  部署 + 验证（同 Phase 6-7）
```

### 补充模式规则

- **不修改 design-tokens.css** — 在现有视觉系统下工作
- **不修改未选中的页面** — 只动用户勾选的页面
- **新增页面遵循现有导航结构** — 在 header/footer 中插入链接，保持导航层级一致
- **新图片 quality 写死 low**，已有图片不重新生成
- **遵循现有 CSS 类名约定** — 新增板块用已有站点的命名风格
- **不修改已有 header/footer 的结构** — 只在导航列表中添加/插入新链接

### 典型场景

| 场景 | 用户说的 | Agent 做的 |
|------|---------|----------|
| 单页不够 | "关于页内容太少" | 审计→补板块→补图→测 |
| 缺页面 | "没有工厂页和品质页" | 审计→生成新页→补导航链接→测 |
| 全站补强 | "这个站整体内容偏少" | 审计全站→缺口报告→用户选→批量补 |
| 老站升级 | "aurion-watches 按 v11 标准补齐" | 审计对照新表→缺口报告→补 |
...
| Phase S0 | 站点目录不在工作空间 | 询问用户提供路径 |
| Phase S3 | 已有 CSS 类名风格不一致 | 检查 design-tokens.css 和已有页面的命名模式，匹配之 |
