...
| 页面 | 必须承载的业务内容 |
|------|-------------------|
| 首页 | Hero + 认证徽标条 + 核心能力卡片(3-4张产品图) + 产能数据亮点条(含背景图) + PCB能力图文分屏 + SMT装配图文分屏(reversed) + 应用方案网格(4张概念图) + 工艺流程(大幅内联图) + 为什么选择我们(纹理背景) + CTA卡片(背景图) — **≥10板块, ≥10张图片** |
...
| 角色 | Prompt 重点 | 典型写法 |
|------|-----------|---------|
| hero-bg | **高端产品微距摄影**（强烈推荐）。品类核心产品 extreme close-up，浅景深虚化背景，电影级布光，品牌色氛围。上半部留空给居中文字 | `Premium product macro photography: [品类核心产品] in extreme close-up, [品牌色] finish with [金属色] accents under cinematic lighting, shallow depth of field with [环境背景] softly blurred in background, luxury electronics advertising style, dark navy background with electric blue glow, upper portion clean for centered white text overlay, no people faces, no text, no logos` |
...
#### Hero 背景图 Prompt 策略（关键）

Hero 背景图是整站第一印象，也是最容易出问题的图片。实战经验：

**❌ 失败的 prompt 模式**：
- "PCB manufacturing facility with automated production lines..." → 生成暗沉、工业感过重、缺乏层次
- 纯文字描述工厂/车间 → AI 生成结果太平淡、没有视觉焦点
- 抽象科技纹理 → 缺乏"产品感"，无法传达业务价值

**✅ 成功的 prompt 模式（高端产品微距摄影）**：
```
Premium product macro photography: [品类核心产品] in extreme close-up,
[品牌主色] circuit traces with [金属色] pads under cinematic lighting,
shallow depth of field with [环境] softly blurred in background,
luxury electronics advertising style, rich contrast,
dark navy background with electric blue accents,
upper area clean for centered white text overlay,
no people faces, no text, no logos
```

**具体示例（PCB/PCBA 站）**：
```
Premium product macro photography: multi-layer PCB with gold ENIG finish
in extreme close-up, deep blue solder mask with intricate copper traces
catching golden rim light, shallow depth of field with SMT production line
softly blurred in background, luxury electronics advertising style,
rich contrast, dark navy background with electric blue circuit glow,
upper portion clean for centered white text overlay,
no people faces, no text, no logos
```
...
| 要素 | 作用 | 示例 |
|------|------|------|
| "premium product macro photography" | 定调高端产品摄影，非工业快照 | 必须放在 prompt 开头 |
| "extreme close-up" | AI 生产微距特写，有视觉冲击力 | 替代"factory environment" |
| "shallow depth of field" | 前景清晰 + 背景虚化 = 层次感 | 用 "softly blurred in background" 描述背景 |
| "cinematic lighting" | 电影级布光，非平光 | 搭配 "golden rim light" / "electric blue glow" |
| "luxury electronics advertising style" | 广告级画面质量 | 替代 "professional B2B photography" |
| "upper area/portion clean for text overlay" | 为文字留空间 | 明确文字位置（centered / left-aligned） |
| "dark [品牌色] background with [强调色] accents" | 匹配品牌色 | 用具体的色彩描述 |
| "no people faces, no text, no logos" | 避免 AI 生成垃圾文字/Logo | 所有 prompt 通用规则 |

#### Hero 图片迭代流程

如果 Hero 图生成效果不理想（暗沉、工业感过重、缺乏高端感），**不要在原 prompt 上微调**。执行以下流程：

1. **分析失败原因**：是构图问题（太广角/太杂乱）、色调问题（太暗/太暖/太冷）、还是质感问题（不够高端/太工业风）？
2. **生成 3 个差异化 prompt 变体**：
   - Variant A：微距产品特写（推荐首选）
   - Variant B：产品 + 虚化车间背景（人像模式替代）
   - Variant C：干净工作室布光 + 品牌色渐变氛围
3. **并行生成 3 张图片**，让用户选择
4. **保留最佳版本**，另外 2 张可降级为 sub-hero-bg 或 cta-bg 使用
5. 如果 3 张都不满意 → 回到步骤 1 分析更根本的问题（品类视觉方向可能选错了）

> **踩坑经验**：Whale PCBA 项目经过 3 轮 Hero 迭代（抽象蓝色 → 暗沉工厂 → 金色 ENIG PCB 微距），最终发现「高端产品微距摄影」prompt 策略比「工厂环境」策略效果好得多。B2B 电子制造站 Hero 的关键是**让采购经理看到你的产品有多精密**，而不是你的车间有多大。

### 首页图片密度与板块编排

#### 核心原则：首页是整站视觉密度最高的页面

实战验证：B2B 采购经理扫描首页时，**每向下滚动一次应该看到新的图片和新的内容板块**。纯文字或单张图配大片文字的 layout 显得内容单薄、缺乏实力感。

**硬性标准**：
- 首页图片 ≥ **10-12 张**（不含 logo/favicon/icon）
- 首页板块 ≥ **9-10 个**（每个板块有独立的视觉呈现）
- 每个板块距上一个板块之间有明确的视觉分隔（背景色切换 / 图片 / 数据条）

#### 推荐首页板块结构模板

```
Section 1  Hero (全幅背景图)                     ← hero-bg ×1
Section 2  认证/信任徽标条 (纯CSS/文字)            ← 无图，快速传递信任信号
Section 3  核心能力卡片 (3-4张产品图)              ← product-showcase ×3-4
Section 4  数据亮点条 (深色背景 + 统计数值)        ← stats-bg ×1
Section 5  PCB/产品 图文分屏 (左文右图)            ← product-showcase ×1
Section 6  SMT/装配 图文分屏 (左图右文, reversed)  ← product-showcase ×1
Section 7  应用方案网格 (4张概念场景图)            ← concept-scene ×4
Section 8  工艺流程图 (大幅内联图 + 步骤文字)      ← process-flow ×1
Section 9  为什么选择我们 (纹理背景 + 多卡片)      ← texture-bg ×1
Section 10 CTA 卡片 (背景图 + 表单按钮)            ← cta-bg ×1
```
...
以下组件在 Whale PCBA 实战中验证有效，建议每个站点直接复用：

**能力卡片（带产品图）** `.capability-img-card`：
```css
.capability-img-card{
...
.capability-img-card:hover{
  transform:translateY(-4px);
  box-shadow:var(--shadow-xl);
}
.capability-img-wrap{width:100%;height:200px;overflow:hidden}
.capability-img-wrap img{
  width:100%;height:100%;object-fit:cover;
  transition:transform var(--transition-slow);
}
.capability-img-card:hover .capability-img-wrap img{transform:scale(1.05)}
```

**应用方案网格** `.app-grid` / `.app-card`：
```css
...
.app-card-img{width:100%;height:180px;overflow:hidden}
.app-card-img img{width:100%;height:100%;object-fit:cover;transition:transform var(--transition-slow)}
.app-card:hover .app-card-img img{transform:scale(1.05)}
.app-card-body{padding:var(--space-md) var(--space-lg)}
.app-card-body h4{font-size:var(--text-base);margin-bottom:2px;color:var(--text-primary)}
.app-card-body p{font-size:var(--text-xs);color:var(--text-secondary)}
...
**全幅数据条（带背景图）** `.stats-bar-full`：
```css
.stats-bar-full{
  padding:var(--space-4xl) 0;
  background:var(--gradient-hero);  /* 深色渐变兜底 */
  position:relative;
  overflow:hidden;
}
.stats-bar-full::before{
  content:'';position:absolute;inset:0;
  background:url(/generated/stats-bg.webp) center/cover no-repeat;
  opacity:.25;z-index:0;
}
.stats-bar-full .container{position:relative;z-index:1}
.stats-bar-full .stat-value{color:#fff}
.stats-bar-full .stat-label{color:rgba(255,255,255,0.7)}
```

**垂直流程列表** `.process-list` / `.process-num`：
```css
.process-list{display:flex;flex-direction:column;gap:var(--space-lg)}
.process-list-item{display:flex;gap:var(--space-md);align-items:flex-start}
.process-num{
  flex-shrink:0;
...
.process-list-item strong{display:block;font-size:var(--text-sm);font-weight:600;color:var(--text-primary);margin-bottom:2px}
.process-list-item span{font-size:var(--text-sm);color:var(--text-secondary);line-height:1.6}
```
...
| 板块 | 背景 | 密度 | 节奏感 |
|------|------|------|--------|
| Hero | 深色全幅图 | 低（大字+按钮） | 开场 |
| 认证条 | 白色/浅灰 | 极低（一排 logo/文字） | 呼吸 |
| 能力卡片 | 白色 | 中（3-4 卡片+图） | 推进 |
| 数据条 | 深色+图 | 中（4 个数字） | 重音 |
| 图文分屏×2 | 白色 | 中高（图+文字+参数） | 推进 |
| 应用方案网格 | 白色 | 高（4 卡片+图+文字） | 信息密集 |
| 工艺流程 | 浅灰 | 中（大图+步骤列表） | 呼吸 |
| Why Choose Us | 纹理背景 | 中（多卡片） | 说服 |
| CTA | 深色+图 | 低（大字+按钮） | 收尾 |

> **踩坑经验**：Whale PCBA 初版首页只有 7 个板块 + 3 张图，用户反馈「首页图片太少了，多一些板块」。从 3 张图扩展到 12 张图、7 板块扩展到 10 板块后，首页的视觉说服力显著提升。B2B 采购经理滚动首页时，每个板块的图片帮助你「展示」而非「描述」你的能力。
