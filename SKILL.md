...
name: 建站工作流
version: "11"
description: B2B外贸出口建站（通用）。一句话→完整页面树→SEO+安全基础设施→全站图片策略→10秒买家测试→部署后审计。纯静态HTML多页站（30+页）。博客只需3篇。适用任意制造/贸易品类。
...
用户: "做个{品类}外贸站，叫{品牌名}"
  │
  ▼
Phase 0  一句话 → 完整 B2B 内容蓝图 + 页面树（30+页）
  │ 模型知识库自动填充：产品参数/认证/交期/MOQ/产能/工艺
  │ 输出 4 层页面树：核心信任页 + 能力页 + 行业页 + 营销页
  ▼
Phase 1  设计决策 + SEO/安全基础设施 + 全站图片策略
  │ 每页每 section 预先分配图片角色
  │ 品类细分 → 品牌人格三轴 → 差异化防收敛
  │ 产出 _headers / robots.txt / sitemap.xml / llms.txt
  ▼
Phase 2  全站生图（quality 写死 low，$0.006/张，零复用）
  │ 不同角色用不同 prompt 策略
  ▼
Phase 3  逐页编码（含 404 / Google Ads 落地页模板）
  ▼
Phase 4  10秒买家测试 ←──┐
  │ 不通过 → 缺什么视觉模块？   │ 设计迭代循环
...
```
Phase 0  主 Agent → 一句话展开为内容蓝图 + 完整页面树
         ├─ content-blueprint.md（产品参数/认证/交期/产能/工艺/信任信号）
         └─ page-tree.json（4层页面树：核心信任+能力+行业+营销，30+页）

Phase 1  主 Agent → 设计 + SEO/安全基础设施 + 图片策略
         ├─ design-blueprint.md（品类+人格+差异化）
         ├─ design-tokens.css
         ├─ index.html
         ├─ blocks/*.html
         ├─ image-strategy.json（全站图片角色分配 + prompt，零复用）
         ├─ image-brief.html
         ├─ acceptance-criteria.json
         ├─ _headers（安全头：HSTS/X-Frame/CSP）
         ├─ robots.txt（阻止AI爬虫）
         ├─ sitemap.xml（框架）
         └─ llms.txt（AI索引）

Phase 2  GPT Image 2 → /generated/*.webp（全站图片，quality 硬写死 low）
         └─ python skills/ai-image-gen/scripts/gen-image.py --prompts image-strategy.json --out ./generated

Phase 3  主 Agent → 逐页编码（含 404 / Google Ads 落地页模板）
...
`industries/` 为 hub 页，从 Phase 0 自动填充的"应用领域"字段展开：

```
industries/{automotive, medical, industrial, aerospace, consumer, energy, iot, telecom}/index.html
```
...
**生图脚本**：使用独立技能 `skills/ai-image-gen/scripts/gen-image.py`（quality 写死 low，不暴露 `--quality` 参数）。脚本已内置文本生图和图生图自动切换、双端点回退、超时 600s。
...
# 追加图片（Phase 4 迭代补图）
python skills/ai-image-gen/scripts/gen-image.py --prompts supplement.json --out ./generated --manifest image-manifest.json
```
...
主 Agent 直接写所有页面（基于 page-tree.json）：
...
| 12 | **质量 LOCKED LOW：gen-image.py quality 写死 low，4层硬阻止 medium/high，误用即 exit(1)** |
...
| Phase 2 | gen-image.py 不在磁盘 | 确认 `skills/ai-image-gen/scripts/gen-image.py` 存在 |
