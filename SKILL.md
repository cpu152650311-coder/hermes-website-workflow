...
Phase 1  设计决策 + 全站图片策略
  ...
  │ 品类细分 → 品牌人格三轴 → 差异化防收敛
  ▼
Phase 2  全站生图（28-35 张，~$0.21，quality 写死 low）
  │ 不同角色用不同 prompt 策略
  ▼
Phase 3  逐页编码
  ▼
...
模型：**GPT Image 2**。质量**写死 `low`**（已实测，low 和 high 无肉眼可见区别，差价 35 倍）。

> `skills/ai-image-gen/scripts/gen-image.py` 是独立的生图脚本，不暴露 quality 参数，无法误用 high。
